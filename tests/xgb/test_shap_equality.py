...
def test_prediction_decomposition_eqal_eli5():
    ...
    model, xgb_data, data = build_model.build_depth_3_model(return_data = True, drop_columns = columns_to_drop)

    shapley_values_xgb = model.predict(xgb_data, pred_contribs = True)

    shapley_values_xgb = pd.DataFrame(shapley_values_xgb, columns = xgb_data.feature_names + ['bias'])
    ...
    pd.testing.assert_frame_equal(shapley_values_xgb.iloc[[0]], shapley_values, check_dtype = False)
